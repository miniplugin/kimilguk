...
#### 실시간 투표 확인 대시보드(트렌드기술-빅데이터,SNS가 활성화 되면서)
- 투표앱에 접속 A사람 :A가 그래프동일 - A투표를 했음
- 투포앱에 접속 B사람 :B가 그래프동일 - A가 투표했을때 B그래프가 자동으로 변경되야 실시간 대시보드
- 실시간 대시보드 구현: 푸시기능(카카오 알림, 실시간 대시보드, SNS댓글좋아요) - 소켓으로 구현
- 구름IDE에서 노드js컨테이너 열어서 소켓을 구현할 예정.
- A,B 라는 두사람이 웹브라우저(앱)을 띄우면, 노드js 소켓서버에 연결이 되어서, 통화중 상태.
- Micro Service Architecture: 기존 비지니스 로직을 잘게 쪼게서 별도의 서비로 분리 구현하는 기술 
- MSA구현은 RestAPI기술로 구현예: 댓글서비스 API제작
- 개발자 입장에서는 스프링프로젝트를 예를 들면, 우리댓글 기능을 사용하지 않고, 외부 discuss기능 RestAPI가져다 사용하게 커스터마이징하는 역할 개발자역할(예, 네아로 붙이는 작업과 같습니다.)
- 스프링프로젝트에 채팅기능이 필요하면, 외부 RestAPI를 가져다가 붙이는 작업.
- DevOps: Development 개발 과 Operation 운영을 동시에 하는 사람, 직원, 회사원
- 본인회사 자체개발 -> 외주개발+고객사(1년 무상유지, 2-3년 지나면 못사용 버림)
- -> 그래서 나온 해결책 : 자체개발(DevOps트렌드 : 개발해서 운영까지 하겠다)
- DevOps = FullStack개발자 트렌드용어

#### 20211030(토)
- 현재 스프링 프로젝트 자바버전 9이상에서 문제되지 않게 처리(아래)
- pom.xml 에 확인할 주석부분 자바9 이상에서 @Resource 지원하지 않아서 추가

#### 20210419(월) 작업
- 수업전: 레이더 챠트로 변경 작업OK.
- 오픈챠트의 실시간 대시보드 구현: 푸시기능(카카오 알림, 실시간 대시보드, SNS댓글좋아요) - 소켓으로 구현OK
- 구름IDE에서 노드js컨테이너 열어서 소켓을 구현할 예정 변경 -> 헤로쿠로 항상 살아 있게 구현예정.
- 노드js프로젝트 신규로 1개 만들어서, 헤로쿠에 배포하는 과정이 필요OK.
...
- 정리하면, 소켓은 1:1통신, io는 1대 다 통신

#### 20210416(금) 작업
- 오픈챠트의 Json 데이터를 DB에서 받아서 CRUD 처리하기 구현OK.
- 스프링의 투표용 RestAPI서버 생성(JsonDataController 클래스에)OK.

#### 20210415(목) 작업.
- 오픈챠트의 Json 데이터 UI처리 작업OK.
...
- 스프링프로젝트에서는 : SSO구현하지 않고, 네아로처럼 갖다가 쓰는 방식을 아시는 목적.
- 스프링RestAPI와 안드로이드앱과 비동기 통신 진행OK.
...
- https, 로그인 하면, 아이디/암호 전송할때, 사람이 알아볼수 없는 텍스트방식으로 전송,
