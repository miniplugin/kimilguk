...
#### 20210408(목) 작업예정
- 스트립램프에 색상을 흐르게 하기 작업예정
- 코딩테스트09문제 C언어로 다어그램대로 진행예정.
- 코딩테스트10문제.(토스-송금앱의 코딩테스트문제): 자바예정

#### 20210407(수) 작업
- 적외선 센서를 이용해서 LED제어하기 작업OK.
- 코딩테스트09문제 C언어로 실행  진행중OK.
- 자바의 substring메서드의 기능을 C언어에서는 strncpy로 구현.
- 포인터변수: char* w = "$(document)";
- strncpy(u, w, 1); => (w=인덱스0, 1=인덱스) => $
- strncpy(v, w+1, strlen(w)-1); => (인덱스 1, 인덱스 10) => (document)
